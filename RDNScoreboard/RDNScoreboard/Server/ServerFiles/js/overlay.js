...
function startOverlay() {
    ...
    var textSize = getParameterByName("tsize");
    var turnOffTimeOuts= getParameterByName("sto");
    if (gscreen === "")
        gscreen = "00ff01";
    if (t1Color === "")
        t1Color = "872f95";
    ...
    var modBack = "background-color:rgba(" + backgroundColor.r + "," + backgroundColor.g + "," + backgroundColor.b + "," + modtrans + "); " + modTopBottom + ":" + vertical + "px; font-size:" + textSize + "em;";
    ...
    if (turnOffTimeOuts === "off")
        $("#timeoutRow").toggleClass("hidden", true);
}
...
function populateOverlay(jsonObject) {
    if (jsonObject.gameStarted === "True") {
        $("#ClockPeriodTime").html(convertMillisecondsToHuman(jsonObject.periodClock));
        $("#ClockPeriodNumber").html(jsonObject.periodNumber);
        $("#JamNumber").html(jsonObject.jamNumber);
        ...
        if (tot2 == 3)
            $("#timeOut3Team2").toggleClass("timeOutOn", true);
        else
            $("#timeOut3Team2").toggleClass("timeOutOn", false);
        if (tot2 >= 2)
            $("#timeOut2Team2").toggleClass("timeOutOn", true);
        else
            $("#timeOut2Team2").toggleClass("timeOutOn", false);
        if (tot2 >= 1)
            $("#timeOut1Team2").toggleClass("timeOutOn", true);
        else
            $("#timeOut1Team2").toggleClass("timeOutOn", false);

        var tot1 = parseFloat(jsonObject.team1TimeOut);
        if (tot1 == 3)
            $("#timeOut3Team1").toggleClass("timeOutOn", true);
        else
            $("#timeOut3Team1").toggleClass("timeOutOn", false);
        if (tot1 >= 2)
            $("#timeOut2Team1").toggleClass("timeOutOn", true);
        else
            $("#timeOut2Team1").toggleClass("timeOutOn", false);
        if (tot1 >= 1)
            $("#timeOut1Team1").toggleClass("timeOutOn", true);
        else
            $("#timeOut1Team1").toggleClass("timeOutOn", false);
        if (jsonObject.isJamRunning === "True") {
            $("#ClockJamTime").html(convertMillisecondsToHuman(jsonObject.jamClock));
        }
        if (jsonObject.isTimeOutRunning === "True") {
            $("#JamNumber").html("Time Out");
            $("#ClockJamTime").html(convertMillisecondsToHuman(jsonObject.timeOutClock));
        }
        if (jsonObject.isLineUpRunning === "True") {
            $("#JamNumber").html("Line Up");
            $("#ClockJamTime").html(convertMillisecondsToHuman(jsonObject.lineUpClock));
        }
        if (jsonObject.isIntermissionRunning === "True") {
            $("#ClockPeriodNumber").html(jsonObject.intermissionName);
            $("#ClockPeriodTime").html(convertMillisecondsToHuman(jsonObject.intermissionClock));
        }
        if (jsonObject.isJamRunning != "True" && jsonObject.isTimeOutRunning != "True" && jsonObject.isLineUpRunning != "True")
            $("#ClockJamTime").html(convertMillisecondsToHuman(jsonObject.jamClock));
    }
    else {
        $("#ClockPeriodNumber").html("Not Started");
    }
    //we resize the names td to make way for larger names on one side compared to the other.  ENGLAND USA
    var name1 = "";
    var name2 = "";
    var difference = 0;

    if (jsonObject.team1Name != null) {
        name1 = jsonObject.team1Name;
    }
    if (jsonObject.team2Name != null) {
        name2 = jsonObject.team2Name;
    }
    if (name1.length > name2.length) {
        difference = name1.length - name2.length;
        for (var i = 0; i < difference; i++)
            name2 += "&nbsp;&nbsp;";
    } else if (name2.length > name1.length) {
        difference = name2.length - name1.length;
        for (var i = 0; i < difference; i++)
            name1 += "&nbsp;&nbsp;";
    }
    $("#Team1Name").html(name1);
    $("#Team2Name").html(name2);
    if (jsonObject.team1Score != null)
        $("#Team1Score").html(jsonObject.team1Score);
    ...
    if (jsonObject.team2Score != null)
        $("#Team2Score").html(jsonObject.team2Score);
