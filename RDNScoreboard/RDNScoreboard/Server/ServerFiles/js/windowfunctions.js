...
_windowFunctions = {
    ...
    getAspectDimensionsObject: function (container, aspect, overflow) {
        var width, height, top, bottom, left, right;
        if ((aspect > ($(container).width() / $(container).height())) == (overflow == true)) {
            width = Math.round((aspect * $(container).height()));
            height = $(container).height();
            top = bottom = 0;
            left = right = (($(container).width() - width) / 2);
        } else {
            width = $(container).width();
            height = Math.round(($(container).width() / aspect));
            top = bottom = (($(container).height() - height) / 2);
            left = right = 0;
        }
        return { width: width, height: height, top: top, bottom: bottom, left: left, right: right };
    },
    ...
    get4x3DimensionsObject: function (container, overflow) { return this.getAspectDimensions(container,4 / 3, overflow); },
    get16x9DimensionsObject: function (container, overflow) { return this.getAspectDimensions(container,16 / 9, overflow); },
