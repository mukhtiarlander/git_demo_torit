...
{
    /// <summary>
    /// http://www.thomaslevesque.com/tag/gif/
    /// http://wpfanimatedgif.codeplex.com/
    /// switched the the library called WpfAnimatedGif
    /// </summary>
    public static class ImageBehavior1
    {
        ...
        private static BitmapDecoder GetDecoder(BitmapSource image)
        {
            ...
            try
            {
                ...
                if (decoder == null)
                {
                    ...
                    if (bmp != null)
                    {
                        Logger.Instance.logMessage("image:" + bmp.UriSource.ToString(), LoggerEnum.message);
            ...
            {
                ErrorViewModel.Save(exception, exception.GetType(), ErrorGroupEnum.UI, additionalInformation: Logger.Instance.getLoggedMessages());
            }
            return decoder;
        }
        ...
        private static BitmapSource MakeFrame(BitmapSource fullImage, BitmapSource rawFrame, FrameInfo frameInfo, BitmapSource previousFrame, FrameInfo previousFrameInfo)
        {
            ...
            try
            {
                ...
                //TODO: Fix when new error comes in.
                //Method: FinalizeCreation
                //Insufficient memory to continue the execution of the program.
                bitmap = new RenderTargetBitmap(fullImage.PixelWidth, fullImage.PixelHeight, fullImage.DpiX, fullImage.DpiY, PixelFormats.Pbgra32);
                bitmap.Render(visual);
            }
            catch (Exception exception)
            {
                ErrorViewModel.Save(exception, exception.GetType(), additionalInformation: "width:" + fullImage.PixelWidth + " height: " + fullImage.PixelHeight + " dpix/y: " + fullImage.DpiX + "/" + fullImage.DpiY);
            }
            return bitmap;
        }
