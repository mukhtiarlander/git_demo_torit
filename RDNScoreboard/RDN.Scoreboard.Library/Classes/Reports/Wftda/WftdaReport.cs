...
using RDN.Utilities.Documents;
using RDN.Portable.Util.Enums;
using Scoreboard.Library.Classes.Reports.Wftda;
...
namespace Scoreboard.Library.Classes.Reports.Excel
{
    public class WftdaReport
    {
        ...
        public WftdaReport SetSaveFileName(string fileName)
        {
            this.fileName = fileName;
            return this;
        }
        public WftdaReport SetOpenFileName(string fileName)
        {
        ...
        #region BoutClock
        private void ExportBoutClock(ExcelPackage p)
        {
            try
            {
                Color lightGray = Color.FromArgb(242, 242, 242);
                Color darkGray = Color.FromArgb(89, 89, 89);
                Color aqua = Color.FromArgb(133, 194, 255);
                Color purple = Color.FromArgb(193, 163, 228);
                Color offPink = Color.FromArgb(255, 232, 255);
                Color tabAqua = Color.FromArgb(0, 255, 255);

                ExcelWorksheet reportSheet = p.Workbook.Worksheets.Add("Bout Clock");
                reportSheet.Name = "Bout Clock"; //Setting Sheet's name
                reportSheet.TabColor = tabAqua;
                ...
                BoutClockPage(lightGray, darkGray, aqua, purple, offPink, reportSheet, ref row, doubleColumn, 1);
                row += 1;
                BoutClockPage(lightGray, darkGray, aqua, purple, offPink, reportSheet, ref row, doubleColumn, 2);

                int column = 1;
                column = SetColumnWidth(reportSheet, column, 6.43);
                column = SetColumnWidth(reportSheet, column, 13.29);
                column = SetColumnWidth(reportSheet, column, 15.71);
                column = SetColumnWidth(reportSheet, column, 11.14);
                column = SetColumnWidth(reportSheet, column, 11.14);
                column = SetColumnWidth(reportSheet, column, 11.14);
                column = SetColumnWidth(reportSheet, column, 11.14);
                column = SetColumnWidth(reportSheet, column, 12.71);
                column = SetColumnWidth(reportSheet, column, 10);
        ...
        private static void BoutClockRow(Color aqua, Color purple, Color offPink, ExcelWorksheet reportSheet, int row, int doubleColumn)
        {
            try
            {
                if (row % 2 == 0)
                    reportSheet.Cells[row, 1 + doubleColumn, row, 9 + doubleColumn].SetBackgroundColor(offPink);
                reportSheet.Cells[row, 1 + doubleColumn].SetBorder(ExcelBorderStyle.Thin);
                reportSheet.Cells[row, 2 + doubleColumn].SetBorder(ExcelBorderStyle.Thin);
                reportSheet.Cells[row, 3 + doubleColumn].SetBorder(ExcelBorderStyle.Thin);
                reportSheet.Cells[row, 4 + doubleColumn].SetBorder(ExcelBorderStyle.Thin);
                reportSheet.Cells[row, 5 + doubleColumn, row, 7 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Thin);
                if (row % 2 == 0)
                {
                    reportSheet.Cells[row, 8 + doubleColumn].SetBorder(ExcelBorderStyle.Thin).SetBackgroundColor(purple);
                    reportSheet.Cells[row, 9 + doubleColumn].SetBorder(ExcelBorderStyle.Thin).SetBackgroundColor(purple);
                }
                else
                {
                    reportSheet.Cells[row, 8 + doubleColumn].SetBorder(ExcelBorderStyle.Thin).SetBackgroundColor(aqua);
                    reportSheet.Cells[row, 9 + doubleColumn].SetBorder(ExcelBorderStyle.Thin).SetBackgroundColor(aqua);
                }
            }
            catch (Exception exception)
            {
        ...
        private static void BoutClockCaptainsLine(string teamName, ExcelWorksheet reportSheet, int row, int doubleColumn)
        {
            try
            {
                reportSheet.Cells[row, 1 + doubleColumn].SetValue("C").SetFontBold().SetCenterAlign().SetBorder(ExcelBorderStyle.Thin);
                reportSheet.Cells[row + 1, 1 + doubleColumn].SetValue("A").SetFontBold().SetCenterAlign().SetBorder(ExcelBorderStyle.Thin);

                reportSheet.Cells[row, 1 + doubleColumn, 1 + row, 4 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[row, 2 + doubleColumn, row, 3 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Thin);
                reportSheet.Cells[row + 1, 2 + doubleColumn, row + 1, 3 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Thin);
                reportSheet.Cells[row, 4 + doubleColumn, 1 + row, 4 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Dotted);
                reportSheet.Cells[row, 5 + doubleColumn, 1 + row, 5 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Dotted);
                reportSheet.Cells[row, 6 + doubleColumn, 1 + row, 6 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Dotted);
                reportSheet.Cells[row, 7 + doubleColumn, 1 + row, 7 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[row, 8 + doubleColumn, 1 + row, 9 + doubleColumn].Merge().SetBorder(ExcelBorderStyle.Medium).SetCenterAlign().SetValue(teamName + " Team");
        ...
        private void ExportPenaltySummary(ExcelPackage p)
        {
            try
            {
                ...
                int doubleColumn = 0;
                int headerColumn = 1;
                reportSheet.Row(row).Height = 20.25;
                ...
                row += 20;
                reportSheet.Row(row).Height = 19.50;
                PenaltiesSummaryFooterAttributes(pink1, pink2, reportSheet, row, doubleColumn);
                reportSheet.Row(25).Height = 19.50;
                reportSheet.Row(26).Height = 19.50;
                reportSheet.Row(27).Height = 19.50;
                reportSheet.Row(28).Height = 12.75;
                reportSheet.Row(29).Height = 12.75;
                ...
                reportSheet.Row(row).Height = 19.50;
                PenaltiesSummaryFooterAttributes(pink1, pink2, reportSheet, row, doubleColumn);
                reportSheet.Row(54).Height = 19.50;
                reportSheet.Row(55).Height = 19.50;
                reportSheet.Row(56).Height = 12.75;
                reportSheet.Row(57).Height = 12.75;
                reportSheet.Row(58).Height = 12.75;

                PenaltySummaryBorders(reportSheet, doubleColumn);

                PenaltySummaryColumnWidths(reportSheet);
            }
            catch (Exception exception)
            {
                ErrorViewModel.Save(exception, exception.GetType());
        ...
        private void PenaltySummaryColumnWidths(ExcelWorksheet reportSheet)
        {
            try
            {
                int column = 1;
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 20);
                column = SetColumnWidth(reportSheet, column, 4);//c
                column = SetColumnWidth(reportSheet, column, 3);
                column = SetColumnWidth(reportSheet, column, 3);
                column = SetColumnWidth(reportSheet, column, 3);
                column = SetColumnWidth(reportSheet, column, 3);
                column = SetColumnWidth(reportSheet, column, 3);
                ...
                column = SetColumnWidth(reportSheet, column, 3);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 4.71);
                column = SetColumnWidth(reportSheet, column, 8.57);
                column = SetColumnWidth(reportSheet, column, 4.43);
                column = SetColumnWidth(reportSheet, column, 4.43);
                column = SetColumnWidth(reportSheet, column, 6);
            }
            catch (Exception exception)
            {
                ErrorViewModel.Save(exception, exception.GetType());
        ...
        private static void PenaltySummaryBorders(ExcelWorksheet reportSheet, int doubleColumn)
        {
            try
            {
                ////borders
                reportSheet.Cells[28, 12 + doubleColumn, 29, 12 + doubleColumn].Style.Border.Left.Style = ExcelBorderStyle.Thin;
                reportSheet.Cells[57, 12 + doubleColumn, 58, 12 + doubleColumn].Style.Border.Left.Style = ExcelBorderStyle.Thin;
                reportSheet.Cells[1, 1 + doubleColumn, 29, 25 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[30, 1 + doubleColumn, 58, 25 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[2, 3 + doubleColumn, 26, 3 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[31, 3 + doubleColumn, 55, 3 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[2, 4 + doubleColumn, 26, 20 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[31, 4 + doubleColumn, 55, 20 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[2, 22 + doubleColumn, 26, 22 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[31, 22 + doubleColumn, 52, 22 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                //reportSheet.Cells[2, 22 + doubleColumn, row - 1, 22 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[2, 23 + doubleColumn, 23, 25 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[31, 23 + doubleColumn, 52, 25 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
        ...
        private void ExportBoutSummary(ExcelPackage p)
        {
            try
            {
                ...
                row += 1;
                reportSheet.Row(row).Height = 63.75;
                ...
                BoutSummaryFooters(pink1, reportSheet, row, doubleColumn, ref headerColumn);
                //setting borders
                BoutSummaryBorders(28, reportSheet, row, doubleColumn, headerColumn);

                BoutSummarySetColumnWidths(reportSheet);
        ...
        private void BoutSummarySetColumnWidths(ExcelWorksheet reportSheet)
        {
            int column = 1;
            column = SetColumnWidth(reportSheet, column, 6.71);
            column = SetColumnWidth(reportSheet, column, 21.57);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 5.71);//n
            column = SetColumnWidth(reportSheet, column, 4.14);
            column = SetColumnWidth(reportSheet, column, 6.14);//p
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 5.71);//u
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 5.29);//z
            column = SetColumnWidth(reportSheet, column, 5.29);
            column = SetColumnWidth(reportSheet, column, 5.29);
            column = SetColumnWidth(reportSheet, column, 5.29);
            column = SetColumnWidth(reportSheet, column, 5.29);
            column = SetColumnWidth(reportSheet, column, 5.29);
            column = SetColumnWidth(reportSheet, column, 5.29);
            column = SetColumnWidth(reportSheet, column, 5.29);
            column = SetColumnWidth(reportSheet, column, 4);//AH
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 6.71);
            column = SetColumnWidth(reportSheet, column, 21.29);//ak
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 3.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 2.57);
            column = SetColumnWidth(reportSheet, column, 3.57);//AW
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 4.57);
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 4.57);
            column = SetColumnWidth(reportSheet, column, 5.71);//bc
            column = SetColumnWidth(reportSheet, column, 4.57);
            column = SetColumnWidth(reportSheet, column, 5.71);//be
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 4);
            column = SetColumnWidth(reportSheet, column, 5.71);//bl
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 5.71);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);
            column = SetColumnWidth(reportSheet, column, 2.43);//bs
            column = SetColumnWidth(reportSheet, column, 3.71);
        }

        private static void BoutSummaryBorders(int begginngRow, ExcelWorksheet reportSheet, int row, int doubleColumn, int headerColumn)
        {
            try
            {
                reportSheet.Cells[begginngRow, 1 + doubleColumn, row, headerColumn + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 8 + doubleColumn, row, 9 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 17 + doubleColumn, row, 26 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 34 + doubleColumn, row, 35 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 36 + doubleColumn, row, 37 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 38 + doubleColumn, row, 43 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 44 + doubleColumn, row, 51 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 52 + doubleColumn, row, 57 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 58 + doubleColumn, row, 66 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
                reportSheet.Cells[begginngRow, 67 + doubleColumn, row, 72 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
        ...
        private static void BoutSummaryHeaderAttributes(string teamName, Color pink1, ExcelWorksheet reportSheet, int row, int doubleColumn, ref int headerColumn)
        {
            try
            {
                reportSheet.Cells[row, 1 + doubleColumn].SetValue("#").SetBackgroundColor(pink1).SetBorder(ExcelBorderStyle.Thin).SetFontSize(11).SetCenterBottom();
        ...
        private void ExportLineups(ExcelPackage p)
        {
            try
            {
                ...
                int column = 1;
                column = LineUpsColumnWidths(reportSheet, column);
                column = LineUpsColumnWidths(reportSheet, column);
        ...
        private int LineUpsColumnWidths(ExcelWorksheet reportSheet, int column)
        {
            try
            {
                column = SetColumnWidth(reportSheet, column, 7);
                column = SetColumnWidth(reportSheet, column, 5.43);
                column = SetColumnWidth(reportSheet, column, 12);
                column = SetColumnWidth(reportSheet, column, 2);
                column = SetColumnWidth(reportSheet, column, 2);
                column = SetColumnWidth(reportSheet, column, 12);
                column = SetColumnWidth(reportSheet, column, 2);
                column = SetColumnWidth(reportSheet, column, 2);
                ...
                column = SetColumnWidth(reportSheet, column, 12);
                column = SetColumnWidth(reportSheet, column, 2);
                column = SetColumnWidth(reportSheet, column, 2);//q
                column = SetColumnWidth(reportSheet, column, 10.43);
                column = SetColumnWidth(reportSheet, column, 0);
                column = SetColumnWidth(reportSheet, column, 2);
                column = SetColumnWidth(reportSheet, column, 6.29);
                column = SetColumnWidth(reportSheet, column, 18);//V
            }
            catch (Exception exception)
            {
                ErrorViewModel.Save(exception, exception.GetType());
        ...
        private void ExportPenalties(ExcelPackage p)
        {
            try
            {
                ...
                int column = 1;
                column = PenaltiesSetColumnWidths(reportSheet, column);
                column = PenaltiesSetColumnWidths(reportSheet, column);
        ...
        private int PenaltiesSetColumnWidths(ExcelWorksheet reportSheet, int column)
        {
            try
            {
                column = SetColumnWidth(reportSheet, column, 9.14);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);//h
                column = SetColumnWidth(reportSheet, column, 7);
                column = SetColumnWidth(reportSheet, column, 7.71);
                column = SetColumnWidth(reportSheet, column, 14);
                column = SetColumnWidth(reportSheet, column, 14);
                column = SetColumnWidth(reportSheet, column, 14);//M
                column = SetColumnWidth(reportSheet, column, 9.14);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                ...
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);
                column = SetColumnWidth(reportSheet, column, 5);//u
                column = SetColumnWidth(reportSheet, column, 7);
                column = SetColumnWidth(reportSheet, column, 7.43);
                column = SetColumnWidth(reportSheet, column, 9.43);
            }
            catch (Exception exception)
            {
                ErrorViewModel.Save(exception, exception.GetType());
        ...
        private static void DisplayPenaltyCodes(ExcelWorksheet reportSheet, int rowNumber, int doubleColumn)
        {
            ...
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Back_Block).WrapText();
            ...
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.High_Block).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Low_Block);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Low_Block).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Elbows);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Elbows).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Forearms);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Forearms).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Blocking_With_Head);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Blocking_With_Head).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Multi_Player_Block);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Multi_Player_Block).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Blocking_Out_Of_Bounds);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Blocking_Out_Of_Bounds).WrapText();
            reportSheet.Cells[row + 2, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Assisting_Out_Of_Bounds).WrapText();
            ...
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Direction_Of_Gameplay).WrapText();
            reportSheet.Cells[row + 2, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Clockwise_To_Block).WrapText();
            ...
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Out_Of_Play).WrapText();
            reportSheet.Cells[row + 2, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Destruction_Of_Pack).WrapText();
            reportSheet.Cells[row + 3, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Failure_To_Reform).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 3, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 4;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Cut_Track);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Cut_Track).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Skating_Out_Of_Bounds);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Skating_Out_Of_Bounds).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Illegal_Procedure);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Illegal_Procedure).WrapText();
            reportSheet.Cells[row + 2, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.False_Start).WrapText();
            reportSheet.Cells[row + 3, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Violation).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 3, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 4;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Insubordination);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Insubordination).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Delay_Of_Game);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Delay_Of_Game).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 1, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
            row += 2;

            reportSheet.Cells[row, 24 + doubleColumn].SetPenaltyAbbrieviation(PenaltiesWFTDAEnum.Misconduct_Gross);
            reportSheet.Cells[row + 1, 24 + doubleColumn].SetPenaltyName(PenaltiesWFTDAEnum.Misconduct_Gross).WrapText();
            reportSheet.Cells[row, 24 + doubleColumn, row + 2, 24 + doubleColumn].SetBorder(ExcelBorderStyle.Medium);
        ...
        private void ExportScores(ExcelPackage p)
        {
            try
            {
                ...
                int row = 1;
                ScoresFillTeamSheet(green1, green2, greenDark, reportSheet, ref row, 1);
                row = 87;
                ScoresFillTeamSheet(green1, green2, greenDark, reportSheet, ref row, 2);

                int column = 1;
                column = SetColumnWidth(reportSheet, column, 7);
                column = SetColumnWidth(reportSheet, column, 13);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);//G
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                ...
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);//p
                column = SetColumnWidth(reportSheet, column, 10.29);//Q 
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 0);//S
                column = SetColumnWidth(reportSheet, column, 7);//T
                column = SetColumnWidth(reportSheet, column, 13);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);
                column = SetColumnWidth(reportSheet, column, 2.71);//z
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                ...
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);
                column = SetColumnWidth(reportSheet, column, 10.29);//AK
        ...
        private static void ScoresRowHeaderOfSheet(ExcelWorksheet reportSheet, int row, int doubleColumn)
        {
            try
            {
                ...
                reportSheet.Cells[row, 2 + doubleColumn].SetValue("Jammer's Number").SetTitleRowScoresPage().WrapText();
        ...
        private static void ScoresRowQuarterOfSheet(Color green1, Color green2, ExcelWorksheet reportSheet, int row, int quarterColumnStart, int modulatorRow)
        {
            try
            {
        ...
        private void ExportIBRF(ExcelPackage p)
        {
            try
            {
                ...
                int column = 1;
                column = SetColumnWidth(reportSheet, column, 12);
                column = SetColumnWidth(reportSheet, column, 11);
                column = SetColumnWidth(reportSheet, column, 11);
                column = SetColumnWidth(reportSheet, column, 8.71);
                column = SetColumnWidth(reportSheet, column, 5.00);
                column = SetColumnWidth(reportSheet, column, 4.00);
                column = SetColumnWidth(reportSheet, column, 8.00);
                column = SetColumnWidth(reportSheet, column, 11);
                column = SetColumnWidth(reportSheet, column, 11);
                column = SetColumnWidth(reportSheet, column, 8.71);
                column = SetColumnWidth(reportSheet, column, 3.71);
