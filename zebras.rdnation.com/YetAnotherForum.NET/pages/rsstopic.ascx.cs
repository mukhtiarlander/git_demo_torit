          ...
          [NotNull] object categoryActiveId)
        {
                        ...
                        toActDate))
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    {
                        feed.Authors.Add(
                            SyndicationItemExtensions.NewSyndicationPerson(string.Empty, row["UserID"].ToType<long>()));
                    ...
                    if (row["TopicMovedID"].IsNullOrEmptyDBField())
                    {
                        ...
                        syndicationItems.AddSyndicationItem(
                          ...
                          GetPostLatestContent(
                            ...
                            altItem),
                          null,
                          YafBuildLink.GetLinkNotEscaped(ForumPages.posts, true, "t={0}", row["TopicID"].ToType<int>()),
