	...
	public partial class postmessage : ForumPage
	{
		...
		protected bool IsPostReplyDelay()
		{
					...
					&& this.Get<YafBoardSettings>().PostFloodDelay > 0)
			{
				...
				if (this.Get<IYafSession>().LastPost > DateTime.UtcNow.AddSeconds(-this.Get<YafBoardSettings>().PostFloodDelay)
						&& this.EditMessageID == null)
				{
					this.PageContext.AddLoadMessage(
						this.GetTextFormatted(
							"wait", 
							(this.Get<IYafSession>().LastPost - DateTime.UtcNow.AddSeconds(-this.Get<YafBoardSettings>().PostFloodDelay)).
								Seconds));
					return true;
				}
			}
		...
		protected bool IsPostReplyVerified()
		{
					...
					&& this.TopicSubjectTextBox.Text.Trim().AreAnyWordsOverMaxLength(this.Get<YafBoardSettings>().MaxWordLength))
			{
				...
				{
					this.TopicSubjectTextBox.Text = this.TopicSubjectTextBox.Text.Substring(this.Get<YafBoardSettings>().MaxWordLength);
				}
				...
				if (this.Get<YafBoardSettings>().EnableTopicStatus)
				{
					...
					foreach (DataRow row in LegacyDb.TopicStatus_List(this.PageContext.PageBoardID).Rows)
					{
						...
						this.TopicStatus.Items.Add(
							new ListItem(text.IsSet() ? text : row["DefaultDescription"].ToString(), row["TopicStatusName"].ToString()));
					}
						...
						if (this.Get<IYafSession>().MultiQuoteIds != null)
						{
							...
							foreach (var msg in
								this.Get<IYafSession>().MultiQuoteIds.ToArray().Select(
									id => messages.AsEnumerable().Select(t => new TypedMessageList(t)).Where(m => m.MessageID == id.ToType<int>()))
		...
		protected void PostReply_Click([NotNull] object sender, [NotNull] EventArgs e)
		{
			...
			else
			{
				...
				if (this.Get<YafBoardSettings>().EmailModeratorsOnModeratedPost)
				{
					...
					this.Get<ISendNotification>().ToModeratorsThatMessageNeedsApproval(this.PageContext.PageForumID, (int)messageId);
				}
		...
		private bool CanEditPostCheck([NotNull] TypedMessageList message, DataRow topicInfo)
		{
			...
			return ((!postLocked && !forumInfo["Flags"].BinaryAnd(ForumFlags.Flags.IsLocked)
							 ...
							 && (message.UserID.ToType<int>() == this.PageContext.PageUserID)) || this.PageContext.ForumModeratorAccess)
						 && this.PageContext.ForumEditAccess;
		}
		...
		private void InitEditedPost([NotNull] TypedMessageList currentMessage)
		{
			...
			// add topic link...
			this.PageLinks.AddLink(
				this.Server.HtmlDecode(currentMessage.Topic), YafBuildLink.GetLink(ForumPages.posts, "m={0}", this.EditMessageID));
		...
		private void InitQuotedReply(TypedMessageList message)
		{
			...
			this._forumEditor.Text +=
				"[quote={0};{1}]{2}[/quote]\n\n".FormatWith(
					this.Get<IUserDisplayName>().GetName(message.UserID.ToType<int>()), message.MessageID, messageContent).TrimStart();
		}
