                ...
                if (k == ScoreboardConfig.KEY_FOR_UPLOAD)
                {
                    foreach (string errorFile in Request.Files)
                    {
                        ...
                        if (file.ContentLength > 0)
                        {
                            try
                            {
                                DirectoryInfo dir = new DirectoryInfo(LibraryConfig.DataFolder + ServerManager.SAVE_FEEDBACK_FOLDER);
                                if (!dir.Exists)
                                    dir.Create();
                                string compressedFile = Path.Combine(LibraryConfig.DataFolder + ServerManager.SAVE_FEEDBACK_FOLDER, Path.GetFileName(file.FileName));
                                file.SaveAs(compressedFile);
                                string encryptedFile = Compression.Decompress(new FileInfo(compressedFile));
                                string finalFile = Path.Combine(LibraryConfig.DataFolder + ServerManager.SAVE_FEEDBACK_FOLDER, "feedback" + DateTime.UtcNow.ToString("yyyyMMddHHmmss") + ".xml");
                            ...
                            catch (Exception e)
                            {
                                ...
                                string filePath = Path.Combine(LibraryConfig.DataFolder + ServerManager.SAVE_ERRORS_FOLDER, Path.GetFileName(file.FileName));
                                DirectoryInfo dir = new DirectoryInfo(LibraryConfig.DataFolder + ServerManager.SAVE_ERRORS_FOLDER);
                                if (!dir.Exists)
                                    dir.Create();
                ...
                if (k == ScoreboardConfig.KEY_FOR_UPLOAD)
                {
                    ...
                    foreach (string liveGame in Request.Files)
                    {
                        ...
                        if (file.ContentLength > 0)
                        {
                            try
                            {
                                DirectoryInfo dir = new DirectoryInfo(LibraryConfig.DataFolder + ServerManager.SAVE_LIVE_GAMES_FOLDER);
                                if (!dir.Exists)
                                    dir.Create();
                                string compressedFile = Path.Combine(LibraryConfig.DataFolder + ServerManager.SAVE_LIVE_GAMES_FOLDER, Path.GetFileName(file.FileName));
                                file.SaveAs(compressedFile);
                                string encryptedFile = Compression.Decompress(new FileInfo(compressedFile));
                                string finalFile = Path.Combine(LibraryConfig.DataFolder + ServerManager.SAVE_LIVE_GAMES_FOLDER, "LiveGame" + DateTime.UtcNow.ToString("yyyyMMddHHmmss") + ".xml");
                                Encryption.DecryptFiletoFile(encryptedFile, finalFile);
                            ...
                            catch (Exception e)
                            {
                                ...
                                string filePath = Path.Combine(LibraryConfig.DataFolder + ServerManager.SAVE_LIVE_GAMES_FOLDER, Path.GetFileName(file.FileName));
                                DirectoryInfo dir = new DirectoryInfo(LibraryConfig.DataFolder + ServerManager.SAVE_LIVE_GAMES_FOLDER);
                                if (!dir.Exists)
                                    dir.Create();
                                file.SaveAs(filePath);
                    ...
                    foreach (string errorFile in Request.Files)
                    {
                        ...
                        if (file.ContentLength > 0)
                        {
                            try
                            {
                                ...
                                if (!dir.Exists)
                                    dir.Create();
                                string compressedFile = Path.Combine(LibraryConfig.DataFolder + ServerManager.SAVE_OLD_GAMES_FOLDER, Path.GetFileName(file.FileName));
                                file.SaveAs(compressedFile);
                                string encryptedFile = Compression.Decompress(new FileInfo(compressedFile));
