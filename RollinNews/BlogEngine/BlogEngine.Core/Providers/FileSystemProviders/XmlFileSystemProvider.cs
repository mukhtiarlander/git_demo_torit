...
namespace BlogEngine.Core.Providers
{
    ...
    public partial class XmlFileSystemProvider : BlogFileSystemProvider
    {
        ...
        public override IEnumerable<FileSystem.File> GetFiles(FileSystem.Directory BaseDirectory)
        {
            var aPath = BlogAbsolutePath(BaseDirectory.FullPath);
            var sysDirectory = new DirectoryInfo(aPath);
            return sysDirectory.GetFiles("", SearchOption.AllDirectories).Select(x => GetFile(string.Format("{0}/{1}", BaseDirectory.FullPath, x.Name)));
        }
        public List<FileItem> GetAllImages(FileSystem.Directory BaseDirectory)
        {
            ...
            try
            {
                ...
                var items = sysDirectory.EnumerateFiles("*", SearchOption.AllDirectories).ToList();
                items = items.Where(f => extensions.Contains(f.Extension)).ToList();
                for (int i = 0; i < items.Count; i++)
                {
                    //.Select(x => GetFile(string.Format("{0}/{1}", BaseDirectory.FullPath, x.Name)));

                    var path = "~/" + items[i].DirectoryName.Replace(@"\", "/").Remove(0, items[i].DirectoryName.IndexOf("App_Data"));
                    files.Add(GetFileItem(path + "/" + items[i].Name));
        ...
        public FileItem GetFileItem(string VirtualPath)
        {
            try
            {
                VirtualPath = RelativeFilePath(VirtualPath);
