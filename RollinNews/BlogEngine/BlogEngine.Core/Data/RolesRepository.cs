...
namespace BlogEngine.Core.Data
{
    ...
    public class RolesRepository : IRolesRepository
    {
        ...
        public IEnumerable<RoleItem> Find(int take = 10, int skip = 0, string filter = "", string order = "")
        {
            ...
            roles.AddRange(System.Web.Security.Roles.GetAllRoles().Select(r => new RoleItem { RoleName = r, IsSystemRole = Security.IsSystemRole(r) }));
            ...
            else
            {
                ...
                foreach (var right in Right.GetAllRights())
                {
                    if (right.Flag != Rights.None)
                    {
                        ...
                        {
                            //only admininstrators can have the FUNDS rights
                            if (right.Name != "AllowedToSetMonthlyEarnings")
                                right.AddRole(id);
                            else
                            {
