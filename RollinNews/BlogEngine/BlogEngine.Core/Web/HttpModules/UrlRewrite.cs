namespace BlogEngine.Core.Web.HttpModules
{
    ...
    public class UrlRewrite : IHttpModule
    {
        ...
        private static void Rewrite(HttpContext context, Blog blogInstance, string url, string path)
        {
            ...
            else if (urlContainsFileExtension && url.Contains("/NEWS/"))
            {
            ...
            else if (BlogSettings.Instance.RemoveExtensionsFromUrls && path.StartsWith(blogInstance.RelativeWebRoot + "write-for-rollin-news-faq", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(Path.GetExtension(path)))
            {
                UrlRules.RewriteWriteForFAQRollinNews(context, url);
            }
            else if (BlogSettings.Instance.RemoveExtensionsFromUrls && path.StartsWith(blogInstance.RelativeWebRoot + "write-for-rollin-news", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(Path.GetExtension(path)))
            {
                UrlRules.RewriteWriteForRollinNews(context, url);
            }
            ...
            else if (BlogSettings.Instance.RemoveExtensionsFromUrls && path.StartsWith(blogInstance.RelativeWebRoot + "scores", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(Path.GetExtension(path)))
            {
                UrlRules.RewriteOthers(context, url);
            }
            else
            {
