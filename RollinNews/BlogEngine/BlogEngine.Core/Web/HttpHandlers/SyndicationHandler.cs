    ...
    public class SyndicationHandler : IHttpHandler
    {
        ...
        private static List<IPublishable> GenerateItemList(HttpContext context)
        {
            if (!string.IsNullOrEmpty(context.Request.QueryString["category"]))
            {
                ...
                return Post.GetPostsByCategoryIPublishable(categoryId).ConvertAll(ConvertToIPublishable);
            }
            ...
            if (!string.IsNullOrEmpty(context.Request.QueryString["author"]))
            {
                ...
                return Post.GetPostsByAuthorIPublishable(author).ConvertAll(ConvertToIPublishable);
            }
            ...
            // The latest posts
            return Post.ApplicablePosts.ConvertAll(ConvertToIPublishable).ToList();
