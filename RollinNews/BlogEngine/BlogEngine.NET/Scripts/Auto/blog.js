...
BlogEngine = {
    ...
    addComment: function (preview) {
        ...
        var author = "";
        var email = "";
        var content = encodeURIComponent(BlogEngine.comments.contentBox.value);
        var captcha = "";
    ...
    validateAndSubmitCommentForm: function () {
        if (BlogEngine.comments.contentBox.value.length < 1 || /<[a-z][\s\S]*>/i.test(BlogEngine.comments.contentBox.value)) {
            BlogEngine.$("status").innerHTML = "Required";
            BlogEngine.$("txtContent").focus();
            return false;
        }
        if (BlogEngine.isValid(BlogEngine.comments.contentBox.value)) {
