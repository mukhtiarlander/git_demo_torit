...
namespace RDN.Utilities.Error
{
    ...
    public class ErrorManager
    {
        ...
        /// <param name="parameters">Parameters or variables that whose vaules should be saved. Easiest is to get a list from this class, var v = ErrorManager.GetEmptyErrorList(); and then add the data like this: v.Add( () => myImportantVariable ); Variables, arrays, lists and dictionaries work.</param>
        /// <param name="additionalInformation">Additional information that could be useful.</param>        
        public static void SaveErrorObject(Exception e, Type type, string path, ErrorGroupEnum? errorGroup = null, ErrorSeverityEnum? errorSeverity = null, IList<Expression<Func<object>>> parameters = null, string additionalInformation = null)
        {
            SaveErrorObject(GenerateErrorObject(e, type, errorGroup, errorSeverity, null, null, null, null, parameters, additionalInformation), path);
        }
        ...
        public static ErrorObject LoadErrorObject(string path)
        {
            ...
            output = (ErrorObject)formatter.Deserialize(stream);
            stream.Close();
            return output;
        }
