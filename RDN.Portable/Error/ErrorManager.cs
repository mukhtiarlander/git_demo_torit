    ...
    public class ErrorManager
    {
        ...
        public static void GetErrorObjectStream(ErrorObject errorObject, out Stream stream)
        {
            try
            {
                ...
                Serializer.Serialize(stream, ErrorVersion);
        ...
        //}
        public static Stream SaveErrorObject(ErrorObject errorObject)
        {
            Stream stream = new MemoryStream();
            //Serializer.Serialize(stream, ErrorVersion);
            Serializer.Serialize(stream, errorObject);
        ...
        /// <summary>
        /// Loads a file into an error object
        /// </summary>
        /// <param name="path">The file to load</param>
        /// <returns>The error object</returns>
        //public static ErrorObject LoadErrorObject(string path)
        //{
        //    ErrorObject output;
        //    //IFormatter formatter = new BinaryFormatter();
        //    Stream stream;
        //    RDN.Utilities.Security.Encryption.DecryptFiletoStream(path, out stream);
        //    stream.Position = 0;
        //    var version = Serializer.Deserialize<int>(stream);
        //    Debug.Assert(version == ErrorVersion);
        //    output = Serializer.Deserialize<ErrorObject>(stream);
        //    stream.Dispose();
        //    return output;
        //}
        //public static ErrorObject LoadErrorObjectJson(string path)
        //{
        //    ErrorObject output;
        //    Stream stream;
        //    RDN.Utilities.Security.Encryption.DecryptFiletoStream(path, out stream);
        ...
        /// <returns>The error object</returns>
        public static ErrorObject LoadErrorObject(ref Stream stream)
        {
            stream.Position = 0;
            ErrorObject output = null;
            //var version = Serializer.Deserialize<int>(stream);
            //Debug.Assert(version == ErrorVersion);
            output = Serializer.Deserialize<ErrorObject>(stream);
            stream.Dispose();
        ...
        protected static ErrorObject GenerateErrorObject(Exception e, Type type = null, ErrorGroupEnum? errorGroup = null, ErrorSeverityEnum? errorSeverity = null, IDictionary<string, string> sessionData = null, IDictionary<string, string> cookieData = null, IDictionary<string, string> userData = null, IDictionary<string, string> serverVariablesData = null, IList<Expression<Func<object>>> parameters = null, string additionalInformation = null)
        {
            ...
            if (parameters != null && parameters.Count > 0)
            {
                ...
                foreach (var parameter in parameters)
                {
                    parameterValue.Remove(0, parameterValue.Length);
                    ...
                    try
                    {
                        ...
                        var collection = parameter.Body as IDictionary;
                        if (collection != null)
                        {
                        ...
                        // Check if its a list
                        var list = parameter.Body as IList;
                        if (list != null)
                        {
                        ...
                        {
                            // Save the value
                            detail.Value = parameter.ToString();
                            output.ErrorData.Add(detail);
                        }
                    }
                    catch (Exception)
            ...
            var assembly = type.Assembly;
            output.AssemblyName = MobileTypeEnum.WP7.ToString();
            //output.AssemblyVersion =ServerConfig. assemblyInfo.Version.ToString();
            output.Created = DateTime.UtcNow;
            output.NameSpace = type.Namespace;
            //output.ErrorNameSpace = e.;
